...
class AirportLounge(http.Controller):
    ...
    @http.route('/airport-lounge-form', type='http', auth='public', website=True, methods=['GET', 'POST'], csrf=False)
    def request_airport_lounge_details(self, **post):
        error = {}
        values = {}
        success = False
        booking_ref = False
        ...
        # service_type_ids = request.env.ref("sr_uaa_main.airport_lounge_services").sudo().service_type_ids
        travel_class_ids = request.env['travel.class'].sudo().search([])

        title = request.env.ref("sr_uaa_main.airport_lounge_services").sudo().name
        hours = []
        minutes = []
        ...
        if post and request.httprequest.method == 'POST':
            ...
            if service_type_id:
                if service_type_id.is_arrival:
                    ...
                    if post.get('arrival_date', False) and post.get('arrival_hours_id', False) and post.get(
                        ...
                        # arrival_date_time = arrival_date_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
                        # arrival_date_time = self.convert_local_time_to_utcrt_local_time_to_utc(arrival_date_time, post.get('timezone'))
            ...
            if error:
                ...
                vals = {
                    'uaa_services_id': request.env.ref("sr_uaa_main.airport_lounge_services").sudo().id,
        ...
        return request.render('sr_uaa_main.request_airport_lounge_page', values)
